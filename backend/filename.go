...
func BuildExpectedFilename(trackName, artistName, albumName, albumArtist, releaseDate, filenameFormat, playlistName, playlistOwner string, includeTrackNumber bool, position, discNumber int, useAlbumTrackNumber bool, extra ...string) string {
	sanitizedTrack := SanitizeFilename(trackName)
	...
	if err == nil && settings != nil {
		if fmtVal, ok := settings["audioFormat"].(string); ok && fmtVal != "" {
			fmtVal = strings.ToUpper(strings.TrimSpace(fmtVal))
			if fmtVal != "FLAC" && fmtVal != "WAV" && fmtVal != "LOSSLESS" {
				format = "320kbps"
				if qVal, ok := settings["bitrate"].(string); ok && qVal != "" {
					format = qVal
					if !strings.HasSuffix(format, "kbps") && !strings.HasSuffix(format, "k") {
						format = format + "kbps"
					}
				}
				ext = "." + strings.ToLower(fmtVal)
				if fmtVal == "AAC" {
					ext = ".m4a"
				}
			} else {
	...
	return fmt.Sprintf("%s - %s - Spotune [%s]%s", sanitizedTrack, sanitizedArtist, format, ext)
}
