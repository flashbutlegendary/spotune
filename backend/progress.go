...
func (pw *ProgressWriter) Write(p []byte) (int, error) {
	...
	WaitIfPaused() // Block if download is paused
...
func IsPaused() bool {
	isPausedLock.RLock()
	defer isPausedLock.RUnlock()
	return isPaused
}

func SaveQueueForRecovery() {
	appDir, err := EnsureAppDir()
	if err != nil {
		return
	}
	recoveryPath := filepath.Join(appDir, "recovery.json")

	downloadQueueLock.RLock()
	defer downloadQueueLock.RUnlock()

	data, err := json.MarshalIndent(downloadQueue, "", "  ")
	if err == nil {
		os.WriteFile(recoveryPath, data, 0644)
	}
}

func LoadQueueForRecovery() {
	appDir, err := EnsureAppDir()
	if err != nil {
		return
	}
	...
	var savedQueue []DownloadItem
	if err := json.Unmarshal(data, &savedQueue); err == nil {
		downloadQueueLock.Lock()
		for _, item := range savedQueue {
			if item.Status == StatusQueued || item.Status == StatusDownloading {
				exists := false
				for _, existing := range downloadQueue {
