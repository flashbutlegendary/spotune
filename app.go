...
func (a *App) startup(ctx context.Context) {
	...
	if err := backend.InitHistoryDB("Spotune"); err != nil {
	...
	if err := backend.InitProviderManager(); err != nil {
		fmt.Printf("Failed to init provider manager: %v\n", err)
	}
...
func (a *App) DownloadTrack(req DownloadRequest) (DownloadResponse, error) {
	...
	if req.Service == "" || req.Service == "auto" {
		bestService, err := backend.SelectBestProvider()
		if err != nil {
			fmt.Printf("Warning: failed to select best provider: %v\n", err)
			req.Service = "tidal"
		} else {
			req.Service = bestService
		}
	}
	...
	if alreadyExists {
		...
		go func(fPath, track, artist, album, sID, cover, format, source string) {
			...
			backend.AddHistoryItem(item, "Spotune")
...
func (a *App) ExportFailedDownloads() (string, error) {
	...
	defaultFilename := fmt.Sprintf("Spotune_%s_Failed.txt", time.Now().Format("20060102_150405"))
...
func (a *App) GetDownloadHistory() ([]backend.HistoryItem, error) {
	return backend.GetHistoryItems("Spotune")
}

func (a *App) ClearDownloadHistory() error {
	return backend.ClearHistory("Spotune")
}

func (a *App) DeleteDownloadHistoryItem(id string) error {
	return backend.DeleteHistoryItem(id, "Spotune")
}

func (a *App) GetFetchHistory() ([]backend.FetchHistoryItem, error) {
	return backend.GetFetchHistoryItems("Spotune")
}

func (a *App) AddFetchHistory(item backend.FetchHistoryItem) error {
	return backend.AddFetchHistoryItem(item, "Spotune")
}

func (a *App) ClearFetchHistory() error {
	return backend.ClearFetchHistory("Spotune")
}

func (a *App) DeleteFetchHistoryItem(id string) error {
	return backend.DeleteFetchHistoryItem(id, "Spotune")
}

func (a *App) ClearFetchHistoryByType(itemType string) error {
	return backend.ClearFetchHistoryByType(itemType, "Spotune")
}
